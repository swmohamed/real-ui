...
- Dark mode is NOT high contrast — dark mode still needs tested text,
  component, state, and focus contrast pairs.
...
## Beyond minimums (contextual quality targets)

- Enhanced 7:1 body-text contrast can be valuable for reading and low-vision
  contexts, but it is not a universal visual-style requirement. Measure the
  relevant criterion and test actual users/settings where consequence warrants.
- Focus indicators must be unambiguously visible on every surface. A thicker
  ring with strong adjacent contrast is a robust design-system choice; do not
  mislabel one token recipe as the WCAG 2.4.13 formula.
