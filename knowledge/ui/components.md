...
- Heights: 32 (sm) / 40 (md) / 48 (lg mobile-min) / 56 (xl primary mobile);
...
- Hierarchy: **primary** (filled brand — 1 per view), **secondary**
  ...
  or outline+confirm). Icon buttons: 40×40 target min with accessible name
...
- Height 40–48 (mobile ≥44); label always visible (above, 13–14px medium);
...
- Popovers/menus: keyboard navigable, focus trapped, return focus on close;
  positioning with collision flipping (RTL-aware start/end)
