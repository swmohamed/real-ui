...
Use foundations/experience-evidence.md to record people, context, current
journey/channels, outcomes, success evidence, and assumptions when the supplied
material does not already establish them.
...
personalization, history…) are enumerated HERE or they die silently. Extract
actors/permissions, source and authorship, automation authority, shared-state
and conflict behavior, and durable operation/recovery history when present;
these contracts can disappear even when the visible action survives.
...
Model the experience and product fresh
(foundations/{experience-evidence,product-modeling}.md), SEEDED by
the extracted layers (1.5): entities → top tasks → relationships →
volume → actors/permissions → time/freshness → shared state → automation
authority → consequence/reversibility → source/authorship → content/
information priority → screen contracts → new IA skeleton.
...
Preserve actual entry, handoff, continuity, result, and recovery contracts while
re-deriving their surfaces. Stage 4 verdicts then run on THIS skeleton — the old composition's
...
(redesign/originality.md + visual-dna/dna-selector.md). Derive visual job,
independent axes, identity evidence, and style-blind composition; do not select
a named industry style. UX problems are
...
- EXPERIENCE/OUTCOME: current entry, handoff, continuity, completion,
  accessibility/language, and recovery contracts remain valid or have a
  justified transformation; success measures and guardrails match the stated
  improvement.
- CONTROL/SHARED-STATE/OPERATIONS (when applicable): AI suggestion versus
  committed action, data/source boundaries, permissions and approvals,
  concurrency/conflict/history, and long-running states/recovery remain
  explicit and correct. A happy path with lost authority or recovery = FAIL.
